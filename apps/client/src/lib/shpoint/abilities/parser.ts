import { Ability, AbilityType, detectAbilityType, extractForceCost, requiresAction, extractTrigger, ABILITY_ICONS, DEFAULT_FORCE_COSTS } from './types';

/**
 * Parser for converting legacy ability text into structured Ability objects
 * Handles the current format where all abilities are in one long text block
 */
 ...
 */
function extractAbilityName(text: string): string {
 ...
 */
function generateAbilityId(name: string, index: number): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, '')
    .replace(/\s+/g, '-')
 ...
 */
export function parseAbility(text: string, index: number = 0): Ability {
  ...
  const tags: string[] = [];
  if (cleanText.toLowerCase().includes('force')) tags.push('Force');
  if (cleanText.toLowerCase().includes('combat')) tags.push('Combat');
  if (cleanText.toLowerCase().includes('movement')) tags.push('Movement');
  if (cleanText.toLowerCase().includes('attack')) tags.push('Attack');
  if (cleanText.toLowerCase().includes('defense')) tags.push('Defense');
 ...
 */
export function parseLegacyAbilities(abilities: LegacyAbility[]): Ability[] {
 ...
 */
export function parseAbilityText(text: string): Ability[] {
  if (!text || typeof text !== 'string') {
    return [];
  }
 ...
 */
export function validateAbilities(abilities: Ability[]): { valid: Ability[]; errors: string[] } {
  const valid: Ability[] = [];
  const errors: string[] = [];
  
  for (const ability of abilities) {
    const errors_for_ability: string[] = [];
