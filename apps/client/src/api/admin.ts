...
\f0\fs24 \cf0 const API = import.meta.env.VITE_SERVER_URL || "http://localhost:3001";\
\
...
export async function fetchUsers(params: \{\
  search?: string; role?: Role; page?: number; pageSize?: number; sort?: string;\
\}) \{\
  ...
  return res.json() as Promise<\{ items: AdminUser[]; meta: \{ total:number; page:number; pageSize:number; pages:number \} \}>;\
\}\
\
export async function createUser(body: \{ email: string; name?: string; avatarUrl?: string; role?: Role \}) \{\
...
\}\
\
export async function updateUser(id: string, body: Partial<Pick<AdminUser, "name"|"avatarUrl"|"role">>) \{\
