...
function convertNewSideToOneSide(ns) {
    if (!ns)
        return null;
    let tree;
    let treeEdges;
    ...
    const idToPos = {};
    if (rows > 0 && cols > 0) {
        tree = Array.from({ length: rows }, () => Array.from({ length: cols }, () => null));
    ...
    const toExpertise = (arr) => {
        if (!arr || arr.length === 0)
            return undefined;
        const rec = {};
        for (const x of arr)
            rec[x.value] = x.effects ?? [];
        return rec;
    };
    // --- tracks ---
    ...
    const rawDefenseExp = ns.defense?.expertise ??
        ns.attack?.defenseExpertise ??
        ns.defenseExpertise ??
        ns.expertise?.defense;
    return {
        name: ns.name,
        melee,
        ranged,
...
function renderGlyphLine(tokens, variant = 'default') {
    ...
    return (_jsx("span", { style: { display: "inline-flex", gap: 6, flexWrap: "wrap", alignItems: "center" }, children: tokens.map((t, i) => {
            ...
            if (t.includes("->")) {
                ...
                return (_jsxs(React.Fragment, { children: [shouldBreakLine && _jsx("br", {}), _jsxs("span", { style: { display: "inline-flex", gap: 4, alignItems: "center" }, children: [renderGlyphToken(from.trim(), `${i}-from`, variant), _jsx("span", { style: { color: C.treeNodeFg, fontSize: 12, fontWeight: 700 }, children: "\u2192" }), renderGlyphToken(to.trim(), `${i}-to`, variant)] })] }, i));
            }
            return (_jsxs(React.Fragment, { children: [shouldBreakLine && _jsx("br", {}), _jsxs("span", { style: { display: "inline-flex", alignItems: "center" }, children: [renderGlyphToken(t, i, variant), i < tokens.length - 1 && (_jsx("span", { style: {
                                    color: '#ffffff',
                                    fontSize: 16,
                                    fontWeight: 700,
                                    marginLeft: 6,
...
function StatPill({ label, value, color, }) {
    ...
    return (_jsxs("div", { style: {
            ...
            padding: 10,
            minWidth: 120,
        }, children: [_jsx("div", { style: { fontSize: 12, color: C.sub, marginBottom: 6 }, children: label }), _jsx("div", { style: {
                    height: 34,
                    borderRadius: 8,
                    background: `${clr}20`,
...
/** ====== Sekcja Melee/Ranged ====== */
function TrackCard({ title, tint, track, iconClass, }) {
    return (_jsxs("div", { style: {
            flex: 1,
            border: `1px solid ${C.border}`,
            borderRadius: 12,
            background: C.cardBg,
            padding: 14,
        }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }, children: [!!iconClass && _jsx("span", { className: iconClass }), _jsx("div", { style: { fontWeight: 800 }, children: title })] }), _jsxs("div", { style: { display: "flex", gap: 12 }, children: [_jsx(StatPill, { label: "Dice", value: track?.dice, color: tint === "attack" ? "attack" : "neutral" }), _jsx(StatPill, { label: "Range", value: track?.range, color: "neutral" }), _jsx(StatPill, { label: "Defense", value: track?.defense, color: "defense" })] })] }));
}
/** ====== Combat Tree: SVG łączniki (poziome/pionowe/skośne), z obsługą edges ====== */
function CombatTree({ tree, edges, }) {
    ...
    if (!tree || tree.length === 0)
        return _jsx("div", { style: { color: C.sub }, children: "Brak danych." });
    ...
    return (_jsxs("div", { style: {
            ...
            background: C.cardBg,
                    ...
                    alignItems: "center",
                    marginBottom: 10
                }, children: [_jsx("div", { style: { fontWeight: 700 }, children: "Combat tree" }), _jsxs("div", { style: {
                            display: "flex",
                            gap: 8,
                            alignItems: "center",
                            fontSize: 12
                        }, children: [_jsx("button", { onClick: handleZoomOut, style: {
                                    padding: "4px 8px",
                                    border: `1px solid ${C.border}`,
                                    borderRadius: 4,
                                    background: C.cardBg,
                                    color: C.text,
                                    cursor: "pointer"
                                }, children: "\u2212" }), _jsxs("span", { style: { minWidth: 40, textAlign: "center" }, children: [Math.round(zoom * 100), "%"] }), _jsx("button", { onClick: handleZoomIn, style: {
                                    padding: "4px 8px",
                                    border: `1px solid ${C.border}`,
                                    borderRadius: 4,
                                    background: C.cardBg,
                                    color: C.text,
                                    cursor: "pointer"
                                }, children: "+" }), _jsx("button", { onClick: handleResetZoom, style: {
                                    padding: "4px 8px",
                                    border: `1px solid ${C.border}`,
                                    borderRadius: 4,
                                    background: C.cardBg,
                    ...
                    cursor: isDragging ? "grabbing" : "grab"
                }, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseUp, onWheel: handleWheel, children: _jsxs("div", { ref: containerRef, style: {
                        position: "relative",
                        display: "grid",
                        ...
                        minWidth: "100%"
                            ...
                            return (_jsx("div", { ref: (el) => {
                                    ...
                                    return groups.map((group, groupIndex) => (_jsx("div", { style: { display: "flex", gap: 6, alignItems: "center" }, children: group.map((token, tokenIndex) => {
...
/** ====== Tabelki Expertise ====== */
function ExpertiseTable({ title, data }) {
    ...
    return (_jsxs("div", { style: {
            ...
            padding: 12,
            background: C.cardBg,
        }, children: [_jsx("div", { style: { fontWeight: 700, marginBottom: 8 }, children: title }), _jsx("div", { children: rows.map(([range, effects]) => (_jsxs("div", { style: {
                        display: "grid",
                        gridTemplateColumns: "80px 1fr",
                        padding: "8px 10px",
                        borderTop: `1px solid ${C.border}`,
                        alignItems: "center",
                    }, children: [_jsx("div", { style: { fontWeight: 700, fontSize: 12, color: C.sub }, children: range }), _jsx("div", { children: renderGlyphLine(effects, 'expertise') })] }, range))) })] }));
}
/** ====== Sekcja stron A/B (jeśli w JSON) ====== */
function resolveSide(stance, side) {
    if ("name" in stance)
        return stance || null;
    const sAB = stance;
    if (sAB.A || sAB.B)
        return side === "A" ? sAB.A || null : sAB.B || null;
    const sides = stance.sides;
...
export default function StanceCard({ stance }) {
    ...
    const defaultSide = (hasSides && stance.side) ||
        (Array.isArray(stance.sides) && stance.sides[0]?.id) ||
        "A";
    const [side, setSide] = React.useState(defaultSide);
    const data = hasSides ? resolveSide(stance, side) : stance;
    if (!data) {
        return (_jsx("div", { style: { border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, color: C.sub, background: C.cardBg }, children: "Brak danych stancji." }));
    }
    return (_jsxs("div", { style: {
            display: "grid",
            gap: 16,
            ...
            borderRadius: 12,
            padding: "24px"
        }, children: [_jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center" }, children: [_jsx("h3", { style: { margin: 0, fontSize: 18, fontWeight: 800 }, children: data.name }), hasSides && (_jsx("div", { style: { display: "flex", gap: 8 }, children: ["A", "B"].map((s) => {
                            const active = side === s;
                            return (_jsx("button", { onClick: () => setSide(s), style: {
                        ...
                        }) }))] }), _jsxs("div", { style: { display: "flex", gap: 16 }, children: [_jsx(TrackCard, { title: "Melee", tint: "attack", track: data.melee, iconClass: "sp sp-melee" }), _jsx(TrackCard, { title: "Ranged", tint: "attack", track: data.ranged, iconClass: "sp sp-ranged" })] }), _jsx(CombatTree, { tree: data.tree, edges: data.treeEdges }), _jsxs("div", { style: { display: "flex", gap: 16 }, children: [_jsx(ExpertiseTable, { title: "Expertise \u2022 Melee", data: data.expertise?.melee }), _jsx(ExpertiseTable, { title: "Expertise \u2022 Ranged", data: data.expertise?.ranged }), _jsx(ExpertiseTable, { title: "Expertise \u2022 Defense", data: data.expertise?.defense })] })] }));
}
