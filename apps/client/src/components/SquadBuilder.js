...
function normalizeCard(raw) {
    ...
    const pc = Number(raw.pc ?? raw.squad_points ?? 0);
    const sp = Number(raw.sp ?? raw.squad_points ?? 0);
    ...
    const characterName = raw.characterName || raw.character_name || name;
...
export default function SquadBuilder({ characterCollections = [], onSave }) {
    ...
    const isCharacterAlreadyUsed = (characterId) => {
        const character = cards.find(c => c.id === characterId);
        if (!character)
            return false;
