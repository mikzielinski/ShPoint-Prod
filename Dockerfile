FROM node:20-alpine
WORKDIR /app
# RENDER CACHE BYPASS - v1.4.3 - FORCE NEW BUILD

# Force complete rebuild - no cache
RUN echo "RENDER CACHE BYPASS v1.4.3 - $(date)" > /tmp/render-bypass.txt
...
RUN echo "Cache bust v1.4.3 - $(date)" > /tmp/cache-bust.txt
RUN npm install
...
RUN echo "Build cache bust v1.4.3 - $(date)" > /tmp/build-bust.txt
RUN npm run build
